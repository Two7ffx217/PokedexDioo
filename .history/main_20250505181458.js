...
async function loadPokemonItems(offset, limit) {
    try {
        ...
        if (!pokeApi || !pokeApi.getPokemons) {
            throw new Error("pokeApi.getPokemons não está disponível.");
        }

        const pokemons = await pokeApi.getPokemons(offset, limit);
        if (!pokemons || pokemons.length === 0) {
            throw new Error("Nenhum Pokémon retornado da API.");
        }
...
function createPokemonCard(pokemon) {
    ...
    li.className = `pokemon-card ${pokemon.type || 'unknown'}`;
    li.innerHTML = `
        <span class="number">#${pokemon.number?.toString().padStart(3, '0') || '???'}</span>
        <h2 class="name">${pokemon.name || 'Desconhecido'}</h2>
        <img src="${pokemon.photo || 'assets/pokeball.png'}" 
             alt="${pokemon.name || 'Desconhecido'}" 
             loading="lazy"
